...
class AppStore extends Component {
  state = {search: '', activeTabId: tabsList[0].tabId}

  searchingAppChange = event => {
  ...
  getFilteringTabs = searchResults => {
    const {activeTabId} = this.state
    const filteringTabs = searchResults.filter(
  ...
  render() {
    ...
    const searchResults = this.getSearchResultes()

    const filteringTabs = this.getFilteringTabs(searchResults)
        ...
        <div className="app-store-container">
          ...
          <div className="searchInputCon">
            <input
              ...
              onChange={this.searchingAppChange}
              value={search}
            />
            <img
          ...
          <ul className="unordered-list-tabItem">
            {tabsList.map(eachTab => (
              <TabItem
                eachTabItem={eachTab}
                key={eachTab.tabId}
                displayTextButton={this.displayTextButton}
                isActiveStyle={eachTab.tabId === activeTabId}
              />
            ))}
          </ul>
          <ul className="unordered-list-AppItem">
            {filteringTabs.map(eachapp => (
